$WebResponse = Invoke-WebRequest https://raw.githubusercontent.com/Kahvi-0/EDRAV_ENUM/main/AVEDRFingerPrints.txt
...
foreach($line in $processes) {
    ($WebResponse.tostring() -split "[`r`n]" | Select-String -Pattern "`:$line$")  -replace ':',' is running from service'
}
